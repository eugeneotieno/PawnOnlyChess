...
fun main() {
    ...
    while (!pawnsChess.gameOver && getCommand() != "exit") {
        if (pawnsChess.movePawn(command)) pawnsChess.showBoard()
    }
...
class Chessboard(private val player1: String, private val player2: String) {
    ...
    private val board = Array(8) { i -> Array(8) { if (i == 1) 'w' else if (i == 6) 'b' else ' ' } }
    ...
    private fun updateChessboard(location: String, change: Char) {
        board[numToInt(location[1])][letterToInt(location[0])] = change
    }
